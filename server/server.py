...
def main(
    ...
    async def handle_sse(request):
        ...
        logger.info(f"SSE Request Headers: {json.dumps(headers)}")
        
        if request.method == "POST":
            # If it's a POST to /sse, it might be an initialization request or similar
            # In some MCP implementations, POST to the SSE endpoint is allowed
            return await handle_post_message_200(request)
        ...
        try:
            async def send_wrapper(message):
                if message["type"] == "http.response.start":
                    ...
                    forced_keys = {h[0] for h in new_headers}
                    for k, v in message.get("headers", []):
                        if k.lower() not in forced_keys:
    ...
    async def handle_post_message_200(request):
        # Log request headers and body
        headers = dict(request.headers)
        try:
            body = await request.body()
            logger.info(f"POST Request Headers: {json.dumps(headers)} Body: {body.decode('utf-8')}")
            
            # Create a custom receive function to feed the body back
            async def receive():
                return {"type": "http.request", "body": body, "more_body": False}
            
            # Create a response status tracker
            status_code = 200
            ...
            # Return an empty response as the work was done by sse.handle_post_message
            from starlette.responses import Response
            return Response(status_code=200)
        except Exception as e:
            logger.error(f"Error in handle_post_message_200: {str(e)}")
    ...
    starlette_app = Starlette(
        ...
        routes=[
            ...
            Route("/messages/", endpoint=handle_post_message_200, methods=["POST"]),
        ],
    )
